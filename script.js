window.addEventListener('DOMContentLoaded', function () {
  ...
  // ── 3. Custom emoji pin ───────────────────────────────────────
  //   isActive = true  → solid blue fill  (current city)
  //   isActive = false → white fill       (past city)
  ...
  // ── 5. Add markers + popups ───────────────────────────────────
  const coords = [];

  places.forEach(function (p) {
    const marker = L.marker([p.lat, p.lng], {
      icon: makeIcon(p.emoji, p.isActive)
    }).addTo(map);

    const tagHtml = p.tag
      ? `<span class="popup-tag">${p.tag}</span>`
      : '';
  ...
  // ── 6. Dashed line Thailand → London → New York ───────────────
  L.polyline(coords, {
    color:     '#2d5be3',
    weight:    2,
    dashArray: '8, 10',
